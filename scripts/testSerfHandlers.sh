...
unit_test() {
    ...
    if [ -f ${file} ]; then
        ...
        ACEGLUSTER_MOCK_PATH=${SERF_PATH}/tests/mock_packages/acegluster

        docker run --rm  -e "CGO_ENABLED=0" -v ${SERF_PATH}/.gopath/src/golang.org:/data/src/golang.org -v ${SERF_PATH}/.gopath/src/github.com:/data/src/github.com  -e "GOPATH=/data/" -v ${ACEGLUSTER_MOCK_PATH}:/data/src/acegluster -v ${HELPERS_MOCK_PATH}:/data/src/helpers  -v ${SERF_PATH}/src/member-update-x:/data/src/memberupdatex  -v ${SERF_PATH}/bin:/data/bin   -v ${SERF_PATH}/src:/data/serf ${GO_VERSION} go test -v /data/serf/${file_in_src} /data/serf/${test_file}
...
if [[ -e ${SERF_PATH}/.gopath ]]; then
    ...
    docker run --net=host	 --rm -e "GOPATH=/data/" ${PROXY_ARGS} -v ${SERF_PATH}/.gopath:/data/ ${GO_VERSION} go get golang.org/x/sys/unix
    #echo -e  "${blue} GO ${T_OK_ICON} ${T_RESET} "
    docker run --net=host	 --rm -e "GOPATH=/data/" ${PROXY_ARGS} -v ${SERF_PATH}/.gopath:/data/ ${GO_VERSION} go get github.com/sirupsen/logrus
    #echo -e  "${blue} LOGRUS ${T_OK_ICON} ${T_RESET}"
    docker run --net=host	 --rm -e "GOPATH=/data/" ${PROXY_ARGS} -v ${SERF_PATH}/.gopath:/data/ ${GO_VERSION} go get github.com/hashicorp/serf/client
    #echo -e  "${blue} SERF CLIENT ${T_OK_ICON} ${T_RESET}"
    docker run --net=host	 --rm -e "GOPATH=/data/" ${PROXY_ARGS} -v ${SERF_PATH}/.gopath:/data/ ${GO_VERSION} go get github.com/docker/docker
