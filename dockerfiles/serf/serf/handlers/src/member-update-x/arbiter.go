...
func Arbiter() error {
	acelog = helpers.GetLogger() //For Logging

	acelog.Info(" ******** Running Arbiter ******** ")
	...
	if !helpers.IsValidRole(serfRole) {
		acelog.Error("SERF Tag from Env is invalid. ", serfRole)
		serfRole = ""
	}
	...
	acelog.Debug(" ******** Arbiter completed ******** ")
	return nil
}
...
func handleAsaLeader() error {
	...
	acelog.Debug("SERF_TAG_ROLE set to leader")
	...
	acelog.Debug("Members count in the serf cluster ", membersAliveCount)
...
func removeArbiter() error {
	...
	if err != nil {
		acelog.Error("Error verifying node arbiter node presence.", arbiter, err.Error())
		return err
	}
	...
	if len(arbiter) > 0 {
		...
		// Debug prints
		acelog.Debug("Arbiter node to be demoted and removed " + arbiter + " with  state " + state)

		err = helpers.DemoteNode(arbiter)
		if err != nil {
			acelog.Error("Error in Docker demote for arbiter node ", arbiter, err.Error())
			return err
		}

		err = helpers.RemoveNode(arbiter, true)
		if err != nil {
			acelog.Error("Error in Docker remove for arbiter node ", arbiter, err.Error())
			return err
	...
	aceArbiterContainerVal, err := helpers.GetAllSystemDockerNodes("ace_arbiter")
	if err != nil {
		acelog.Error("Error while checking arbiter container ID. ", err.Error())
		return err
	}

	if len(aceArbiterContainerVal) > 0 {

		err := helpers.RemoveSystemDockerNode("ace_arbiter")
		if err != nil {
			acelog.Error("Error while removing ACE arbiter container. ", err.Error())
			return err
		}
	}
...
func updateArbiter() error {
	...
	checkRwoArbiterVal, err := helpers.GetSystemDockerNode("ace_arbiter")
	if err != nil {
		acelog.Error("Error while checking arbiter container ID. ", err.Error())
		return err
	}
	...
	if err != nil {
		acelog.Error("Error verifying node arbiter node presence.", checkForDockerNode, err.Error())
		return err
	}

	if len(checkRwoArbiterVal) > 0 && len(checkForDockerNode) > 0 {
		acelog.Debug("Arbiter exists on this leader.")
	} else {
		...
		if err != nil {
			acelog.Error("Error while retriving Docker join token. ", err.Error())
		}
		...
		if leaderAddr != "" && dockerSwarmJoinTokenVal != "" {
			...
			if err != nil {
				acelog.Error("Error while running ace arbiter. ", err.Error())
			}

			time.Sleep(2) //wait for ace arbiter container to come up.
			...
			if err != nil {
				acelog.Error("Error joining ace node to the swarm cluster. ", err.Error())
				return err
			}
			...
			if err != nil {
				acelog.Error("Error in getting docker swarm id for the ace arbiter container. ", err.Error())
				return err
			}
			...
			if err != nil {
				acelog.Error("Error while setting Arbiter node status to Drain. ", err.Error())
				return err
			}

		} else {
			acelog.Debug("Leader address/Docker join token is empty.")
			return err
		}
