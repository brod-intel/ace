...
var (
	...
	glusterDockerVolumePath, aceBasePath, dockerTokenPath string
...
func Init() error {
	...
	aceBasePath, ok = os.LookupEnv("ACE_BASE_PATH")
	if !ok {
		return fmt.Errorf("ACE_BASE_PATH")
	}
...
func Gluster() error {
	...
	acelog.Info(" ******** Running Gluster ******** ")

	// read gloabl variables
	err := Init()
	if err != nil {
		acelog.Error("Some ENV variables are assigned. Error: ", err)
		return err
	}
	...
	if err != nil {
		acelog.Error("Unable to retrieve docker container status. Exiting")
		return err
	}

	glusterClusterAddr, err = helpers.GetIPAddr()
	if err != nil {
		acelog.Error("Failed to get network IP:", err)
		return err
	}
	...
	if membersCount == 1 {
		...
		if err != nil {
			acelog.Error("Error while listing volumes ", err)
			return err
		}
		acelog.Debug("Volume count is ", len(vols))
		if len(vols) == 0 {
		...
		for _, vol := range vols {
			...
			acelog.Debug("Starting the volume ", vol.Name)
...
func waitForGlusterContainer() error {
	...
	for {
		...
		if err != nil {
			acelog.Error("Error in checking docker gluster container for status ", err.Error())
			return err
		}
		...
		if len(glusterServerContainerVal) > 0 {
			...
			acelog.Debug("Sleeping for 10 seconds to get gluster docker container up")
...
func checkExistingState() error {
	...
	if err != nil {
		acelog.Error("Error while checking gluster Pool list", err.Error())
		return err
	}
...
func checkOldStateOfGluster() error {
	...
	peers, err := client.PeerStatus()
	if err != nil {
		acelog.Error("Error Getting Gluster Peer Status")
		return err
	}
	...
	if aliveMembers == 1 {
		...
		if ipChanged == true {
			acelog.Debug("Ip has been changed, Deleting the volumes.")
		...
		for {
			...
			if len(partofExistingGluster) == 0 && retryCount < 5 {
				acelog.Debug("Gluster pool list response is empty from the server. Retrying")
		...
		if len(partofExistingGluster) > 0 {
			if strings.Contains(string(partofExistingGluster), glusterUUID) {
				...
				if err != nil {
					acelog.Error("Error while setting up serf tag for gluster ")
					return err
				}
			} else {
				err = purgeOldStateOfGluster()
...
func purgeOldStateOfGluster() error {
	...
	// Debug Prints
	acelog.Debug("purgeOldStateOfGluster: Peers List ", peers)
	for _, peer := range peers {
	...
	if len(connectedPeers) == 0 {
		...
		vols, err := client.ListVolumes()
		if err != nil {
			acelog.Error("Error while List gluster volumes", err.Error())
			return err
		}
		...
		for _, vol := range vols {
			...
			if err != nil {
				acelog.Error("Error while setting server quorum ratio ", err)
			}
			...
			for _, brick := range vol.Bricks {
				...
				ourIP, err := helpers.GetIPAddr()
				if err != nil {
					acelog.Error("Failed to get network up:", err)
					return err
				}

				if !strings.Contains(brick, ourIP) {
					if replica > 0 {
						acelog.Debug("Remove Brick ", brick, " with replica ", replica)

						err = rmBricksFromVol(vol.Name, brick, replica)
						if err != nil {
							acelog.Error("Error Removing Brick ", err)
		...
		for _, p := range otherPeers {
			...
			acelog.Debug("Detach peer ", p, " error ", err)
		}
		...
		acelog.Debug("purgeOldStateOfGluster: Some peers are connected. Not removing bricks.")
	}
...
func manageVolumes(glusterClusterAddr string) error {
	...
	acelog.Debug("Performing peer probe with serf query to the leader ", glusterClusterAddr)
	...
	if strings.Contains(string(glusterConnectedToPeerVal), "having volumes") {
		acelog.Debug("Already part of some other gluster, peer probe failed. need to purge old state")
	...
	if err != nil {
		acelog.Error("Error while doing peer probe with serf query ", err.Error())
		return err
	}
	...
	if strings.Contains(string(glusterConnectedToPeerVal), "success") {
		...
		acelog.Debug("Unable to probe peer from the leader. Will set up serf tags to retry.", glusterClusterAddr)
		serfTagGlusterRetry = serfTagGlusterRetry + 1

		err = helpers.SetGlusterRetryTag(strconv.Itoa(serfTagGlusterRetry))
		if err != nil {
			acelog.Error("Error while setting up serf tags for retry")
			return err
		}
		time.Sleep(2 * time.Second)
...
func createGlusterVolume(glusterClusterAddr string) error {
	...
	if err != nil {
		acelog.Error("Error in creating gluster volume")
		return err
	}
...
func setTag() error {
	...
	if err != nil {
		acelog.Error("Unable to mount GlusterFS at "+glusterMountPath+"/"+glusterVolumeName, err.Error())
		serfTagGlusterRetry = serfTagGlusterRetry + 1
		err = helpers.SetGlusterRetryTag(strconv.Itoa(serfTagGlusterRetry))
		if err != nil {
			acelog.Error("Error in setting serf gluster retry tag for gluster")
			return err
		}
	} else {
		acelog.Debug("Mount point exists ", string(checkFormountPointval))
		peers, err := client.PeerStatus()
		if err != nil {
			acelog.Error("Error Getting Peer Status")
		}
		...
		if err != nil {
			acelog.Error("Error in setting serf tag for gluster")
			return err
		}
...
func handleAppsVolumes(glusterClusterAddr string) error {
	vols, err := client.ListVolumes()
	if err != nil {
		acelog.Error("Error in getting gluster volume list", err.Error())
		return err
	}
	for _, vol := range vols {
...
func SetServerQuorumRatio() error {
	...
	vols, err := client.ListVolumes()
	if err != nil {
		acelog.Error("Error in gluster volume list", err.Error())
		return err
	}
	...
	for _, vol := range vols {
		...
		if err != nil {
			acelog.Error("Error in Enabling server quorum ", err.Error())
			return err
		}
	}
...
func addBricks(volName string, glusterClusterAddr string) error {
	...
	for {
		if retryCount < 30 {
			...
			if err != nil {
				acelog.Error("Error while checking gluster bricks count from gluster volume info ", err.Error())
				return err
			}
			...
			for _, brick := range bricks {
				if strings.Contains(brick.Name, glusterClusterAddr) {
					...
					acelog.Debug("brick Name and glusterClusterAddr", brick.Name, glusterClusterAddr)
					break
				}
			}

			if brickAlreadyExists == true {
				acelog.Debug("Brick already exists, No need to add bricks")
				break
			}
			...
			if err != nil {
				acelog.Error("Error while adding bricks to volume, Sleeping for two seconds before retrying.")
	...
	acelog.Debug("Successfully added Bricks to " + volName)
	return nil
}
...
func rmBricksFromVol(n, b string, r int) error {
	...
	for retry > 0 {
		...
		acelog.Error("Error in RemoveBrick: Trying again", err)
...
func deleteGlusterVolumes() error {
	...
	for _, vol := range vols {
		...
		if err != nil {
			acelog.Error("Removing vol "+vol.Name+" Error: ", err)
			return err
		}
