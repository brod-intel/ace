...
func swarmLeaderInitStackDeploy(serfAdvertiseIface string) error {
	...
	if err != nil {
		acelog.Error("Failed to create the swarm Init:", err)
		return err
	}

	swarmID, err := helpers.GetSwarmNodeID()
	if err != nil {
		acelog.Error("Failed to docker info: ", err)
		return err
	}
	...
	if serfTagsOp != nil {
		acelog.Error("Error Deleting, inprocessTag :", serfTagsOp)
		return serfTagsOp
	}

	acelog.Debug("Swarm id is set and inprocess tag deleted")

	_, err = os.Stat("/opt/stacks")
	if os.IsNotExist(err) {
		acelog.Debug("/opt/stacks doesn't exist, Nothing to deploy")
...
func SetSwarmTag() error {
	...
	acelog.Debug("Docker Op swarmNodeID: ", swarmID)
...
func JoinSwarm(serfAdvertiseIFACE string, serfLeader string, role string) error {
	...
	if err != nil {
		acelog.Error("Error while querying swarm token to leader :", err)
		return err
	}

	swarmToken, err := readToken(role)
	if err != nil {
		acelog.Error("Error while reading swarm token:", err)
		return err
	}
	...
	if err != nil {
		acelog.Error("Failed to Join the swarm")
		return err
	}
	time.Sleep(1 * time.Second)

	_, err = helpers.SerfQuery("godocker", "cleanUpStaleMember")
	if err != nil {
		acelog.Error("Error while querying swarm token to leader :", err)
		return err
	}

	acelog.Debug("Successfully joined the swarm")
	return nil
...
func swarmWorker(serfAdvertiseIFACE string, serfLeader string) error {
	...
	acelog.Debug("Joining Swarm as Worker")
	...
	if err != nil {
		acelog.Error("Node failed to join", err.Error())
		return err
	}
...
func aliveMemberLE3(serfAdvertiseIFACE string, serfLeader string) error {
	...
	if len(swarmID) != 0 && swarmManager == false {
		...
		acelog.Debug("Leave the swarm and rejoin incase if the leader is also changed")
	...
	acelog.Debug("Joining Swarm as manager")
	...
	if leftSwarm == true {
		...
		acelog.Debug("Joined swarm by leaving the old docker swarm. Check for docker node with status down for 25 seconds.")
...
func manageSwarm(serfAdvertiseIface string) error {
	...
	//Debug Print
	acelog.Debug("MemberIPByTagsAndStatus Passed, value of serfLeader: ", string(serfLeader))

	if len(serfLeader) != 0 {
		acelog.Debug("Leader Found")
	...
	//Debug Print
	acelog.Debug("waiting for Leader")
...
func Swarm() error {
	...
	acelog.Debug("serftagRole: ", statusRole)
	...
	if statusRole == "leader" {
		...
		if err != nil {
			acelog.Error("swarmLeaderInitStackDeploy Failed")
			return err
		}

		acelog.Debug("swarmLeaderInitStackDeploy completed")

	} else {
		err = manageSwarm(serfAdvertiseIface)
		if err != nil {
			acelog.Error("manageSwarm failed")
			return err
		}

		acelog.Debug("manageSwarm Completed")
