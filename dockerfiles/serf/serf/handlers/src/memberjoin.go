...
func checkGlusterStatus() error {
	...
	if err != nil {
		acelog.Error("Error in Gluster Peer Status")
	}

	glusterListCount := len(peers)

	acelog.Debug("value of gluster list count", glusterListCount)
	...
	if glusterListCount >= 2 {
		...
		acelog.Debug("Gluster UUID is ", glusterUUID)

		glusterPL, err := helpers.SerfQuery("gluster", "pool list")
		if err != nil {
			acelog.Error("Serf query for gluster pool list to the leader has failed: ", glusterPL)
			return err
		}

		if len(glusterPL) == 0 {
			acelog.Debug("No leader is present, returning from memberjoin for gluster")
			return nil
		}

		//If gluster is part of previous swarm, return
		if strings.Contains(glusterPL, glusterUUID) {
			acelog.Error("Current Member is a part of existing Gluster, Gluster Tag will not be removed.")
		...
		if serfTagsOp != nil {
			acelog.Error("Error of in update Tag, Init: ", serfTagsOp)
			return serfTagsOp
		}
		...
		if serfTagsOp != nil {
			acelog.Error("Error of in Deleting Tag, glusterretry: ", serfTagsOp)
			return serfTagsOp
		}
...
func checkSwarmStatus() error {
	...
	acelog.Debug("If the node is rebooted, Serf query will take time as leader might be executing member-failed. Please wait..")
	serfOutput, err := helpers.SerfQuery("docker", "node ls")
	if err != nil {
		acelog.Error("Error in executing docker node ls")
		return err
	}

	if len(serfOutput) == 0 {
		acelog.Debug("No leader is present, returning from memberjoin for swarm")
		return nil
	}
	...
	if err != nil {
		acelog.Error("Docker empty: ", err)
		return err
	}

	if len(swarmID) != 0 && strings.Contains(serfOutput, swarmID) {
		acelog.Debug("Current Member is a part of existing swarm-worker, Swarm Tag will not be removed.")
		return nil
	}
	...
	serfTagsOp := helpers.SetInitTag(strconv.Itoa(int(time.Now().UnixNano())))
	if serfTagsOp != nil {
		acelog.Error("Error in update Init tag: ", serfTagsOp)
		return serfTagsOp
	}
	...
	if serfTagsOp != nil {
		acelog.Error("Error in deleting init tag: ", serfTagsOp)
		return serfTagsOp
	}
...
// else if there exists any ace_arbiter remove it as there are two leaders;
...
func manageLeader() error {
	...
	if err != nil {
		acelog.Error("Failed to get the leader status ", err.Error())
		return err
	}

	acelog.Debug("grep & stop ace_arbiter if it exists")

	containerID, err := helpers.GetSystemDockerNode("ace_arbiter")
	if err != nil {
		acelog.Error("Failed to get container ID for ace_arbiter", err.Error())
		return err
	}
	...
	if inspectManagerStatus == true {
		...
		if len(containerID) == 0 {
			...
			if err != nil {
				acelog.Error("Failed to leave the node:", err)
				return err
			}
			...
			acelog.Debug("I am already the leader")
			return nil
		}
	}
	...
	if len(containerID) != 0 {
		...
		if err != nil {
			acelog.Error("Failed to remove ace_arbiter ", err.Error())
			return err
		}
	}
	...
	if serfTagsOp != nil {
		acelog.Error("Error in update tag ", serfTagsOp)
		return serfTagsOp
	}

	err = checkSwarmStatus()
	if err != nil {
		acelog.Error("Swarm Status Failed")
		return err
	}

	err = checkGlusterStatus()
	if err != nil {
		acelog.Error("Gluster Status Failed")
		return err
	}
...
func checkSwarmAndGlusterStatus() error {
	...
	err := checkSwarmStatus()
	if err != nil {
		acelog.Error("Swarm Status Failed ", err.Error())
		return err
	}
	err = checkGlusterStatus()
	if err != nil {
		acelog.Error("Gluster Status Failed ", err.Error())
		return err
	}
...
func memberJoinAssignLeaderWorker() error {
	...
	acelog.Debug("Alive members are:", CountMembers)

	if CountMembers <= 1 {

		serfTagsOp := helpers.SetRoleTag("leader")
		if serfTagsOp != nil {
			acelog.Error("Error in update tag, Role ", serfTagsOp)
			return serfTagsOp
		}
		...
		serfTagsOp = helpers.SetInitTag(strconv.Itoa(int(time.Now().UnixNano())))
		if serfTagsOp != nil {
			acelog.Error("Error in update tag, Init ", serfTagsOp)
			return serfTagsOp
		}
...
func memberJoinManageLeaderWorker() error {
	...
	if !helpers.IsValidRole(statusRole) {
		acelog.Error("SERF Tag from Env is invalid. ", statusRole)
		statusRole = ""
	}
	...
	if statusRole == "leader" && inspectManagerStatus == false {
		acelog.Debug("Since the role tag specifies this node is a serf leader but not a swarm leader due to a node reboot.")
		serfTagsOp := helpers.SetRoleTag("worker")
		if serfTagsOp != nil {
			acelog.Error("Error in update tag, Role ", serfTagsOp)
			return serfTagsOp
	...
	acelog.Debug("Number of Leader members are:", countLeaderMembers)
	...
	if statusRole == "leader" && countLeaderMembers > 1 && aliveMembers == 2 {
		...
		if inspectManagerStatus == true {
			...
			acelog.Debug("There are two leaders and current node docker status specifies that this is the true leader. check arbiter for its coverage.")
			err := memberupdatex.Arbiter()
			if err != nil {
				acelog.Debug("Error while executing arbiter.")
			}
		}

		status := manageLeader()
		if status == nil {
			acelog.Debug("Swarm Status And Gluster Status Managed for leader")
			return nil
		}
	}
	...
	if statusRole == "leader" && countLeaderMembers > 1 && aliveMembers > 2 {
		...
		acelog.Debug("Handling of two leaders when alive members greater then 2")
		...
		if inspectManagerStatus == true {
			...
			if err != nil {
				acelog.Error("Error: DockerD is not running. Please restart the node.")
				return err
			}

			acelog.Debug("Number of nodes in the docker cluster:", numberOfNodes)
	...
	if err == nil {
		acelog.Error("Swarm And gluster Status handled")
		return nil
	}
...
func handleRoleAndSwarm() error {
	...
	serfTagsOp = helpers.DeleteSerfTag("swarm")
	if serfTagsOp != nil {
		acelog.Error("Error in deleting swarm tag: ", serfTagsOp)
		return serfTagsOp
	}
...
func main() {
	...
	if helpers.CheckNetworkStatus() != nil {
		acelog.Error("Error: IP is not assigned.")
		return
	}
	...
	if len(serfContext.GlusterTag) == 0 && len(serfContext.SwarmTag) == 0 {
		...
		if err == nil {
			acelog.Debug("Leader or worker is assigned")
		} else {
			acelog.Error("Failed in assigning Leader or worker ", err.Error())
		}
		...
		if CountMembers == 1 {
			...
			if err != nil {
				acelog.Error("Failed to Manage Leader node ", err.Error())
			}
			return
		}

		acelog.Debug("Manage the leader/worker")

		err := memberJoinManageLeaderWorker()
		if err != nil {
			acelog.Error("Failed to Manage the node ", err.Error())
			return
		}
