...
func GetIPAddr() (string, error) {
	...
	for {
		...
		if len(IPAddr) > 0 {
			...
			acelog.Info("IP Address not assigned yet..Looking for IP Address")
...
func CheckNetworkStatus() error {
	...
	acelog.Debug("Checking network Status ...")
	networkDownFlag = false
	for {

		_, err := net.LookupIP(networkIP)
		if err != nil {
			acelog.Error("Network is not up ", err.Error())
			...
			acelog.Debug("Network is up.")
			break
		}
	}

	if networkDownFlag == true {
		acelog.Debug(" Network was offline, delayed start to allow other members to settle.")
...
func ExecuteCommand(cmdvalue string) bool {
	...
	if len(cmdvalue) == 0 {
		acelog.Error("Pass the correct input")
		return false
	}

	cmd, err := exec.Command("sh", "-c", cmdvalue).Output()
	if err != nil {
		acelog.Error("Error in executing the command ", err.Error())
		return false
	}

	acelog.Debug("OutPut Value :", cmd)
...
func CreateDir(path string) error {
	...
	if err != nil {
		acelog.Error("Error in creating directory ", err.Error())
		return err
	}
...
func MountGlusterVolumes() error {
	...
	aceBasePath, _ := os.LookupEnv("ACE_BASE_PATH")
	...
	if len(glusterMountPath) == 0 || len(glusterVolumeName) == 0 || len(aceBasePath) == 0 {
	...
	if err != nil {
		...
		acelog.Debug("Gluster mount point does not exists")
		...
		var glusterVolMount = "system-docker exec --env GLUSTER_CLUSTER_ADDR=" + glusterClusterAddr + "  --env GLUSTER_MOUNT_PATH=" + glusterMountPath + " --env GLUSTER_VOLUME_NAME=" + glusterVolumeName + " " + glusterContainerID + " " + aceBasePath + "/gluster/volume-entrypoint.sh"
		if ExecuteCommand(glusterVolMount) != true {
			acelog.Debug("Error while mounting the gluster volume  ", glusterVolMount)
		}
...
func glusterfsCreds() (string, string, error) {
	...
	if _, err := os.Stat(credsFilePath); os.IsNotExist(err) {
		acelog.Error("GlusterLibClient(): credential file does not exist")
		return "", "", err
	}

	data, err := ioutil.ReadFile(credsFilePath)
	if err != nil {
		acelog.Error("GlusterLibClient(): Error Reading File ", err)
		return "", "", err
	}
...
// GlusterLibClient returns a pointer to acegluster client
func GlusterLibClient() *mg.Client {
	...
	if len(key) == 0 || len(cert) == 0 || len(ca) == 0 {
		acelog.Debug("GlusterLibClientWithTLS(): Need keys for TLS. Default to http")
		tls = false
	}
...
func GetSleepTimeFromEnv(envVariable string) int {
	...
	if len(delaySecondsFromEnv) > 0 {
		...
		acelog.Debug(envVariable + " is not defined in the docker compose. Setting it to 30 seconds.")
		delaySeconds = 30
	}
...
func CreateDirForToken() error {
	...
	if len(glusterVolumeName) == 0 || len(glusterMountPath) == 0 || len(dockerTokenPath) == 0 || len(managerTokenPath) == 0 || len(workerTokenPath) == 0 {
		acelog.Error("Env Variables not defined , ", "GLUSTER_VOLUME_NAME ", glusterVolumeName, " ",
...
func EnableServerQuorum(r *mg.Client, name string) error {
	...
	if err != nil {
		acelog.Error("Error in Enabling server quorum ", err)
		return err
	}
	return nil
}
...
func SetQuorumRatio(r *mg.Client, mode string) error {
	...
	if mode == "cleanup" {
		acelog.Debug("Reset the server quorum ratio.")
		for retry > 0 {
		...
		for retry > 0 {
			...
			acelog.Debug("Update the server quorum ratio to ", ratio)
