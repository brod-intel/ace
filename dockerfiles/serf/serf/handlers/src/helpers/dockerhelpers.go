...
func StackDeploy() {
	...
	// /opt/ace/Application_name/docker-compose.yml
	// /opt/ace/ := Base path where ACE searches for Application compose files.
	...
	for _, app := range apps {
		...
		if app.IsDir() {
			...
			for _, file := range files {
				...
				if err != nil {
					acelog.Error("Error while checking Symlink, ", err.Error())
					continue
				}

				if isSymlink == true {
					acelog.Debug("Encountered a Symlink, Assigning the Absolute path before deploying....")
					filePath = symPath
				}
				...
				if strings.HasSuffix(filePath, ".yml") {
					...
					command := buf.String()
					acelog.Debug("command:", command)

					commandop, _ := exec.Command("sh", "-c", command).CombinedOutput()
					acelog.Debug("Output for stack deploy: ", string(commandop))
				}
...
func SwarmInit(serfAdvertiseIface string, forceInit bool) error {
	...
	if err != nil {
		acelog.Error("Error in Initiating New Client:", err)
		return err
	}
	...
	if err != nil {
		acelog.Error("Failed in creating Swarm:", err)
		return err
	}
...
func SwarmJoin(serfAdvertiseIFace string, token string) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "Timeout was reached") {
			acelog.Debug("Timeout before join..waiting for 30 seconds to complete in background ", err)
			return nil
		}

		acelog.Error("Failed to Join Address", err)
		return err
	}
	return nil
}
...
func SwarmLeave(force bool) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "not part of a swarm") {
			acelog.Debug("Not a part of swarm")
			return nil
		}
...
func GetSwarmNodeID() (string, error) {
	...
	if err != nil {
		acelog.Error("Failed to get Swarm Node ID")
		return "", err
	}

	acelog.Debug("node ID ", info.Swarm.NodeID)
...
func GetNodeStatus(status string) (bool, error) {
	cli, err := client.NewClientWithOpts(client.FromEnv)
	if err != nil {
		acelog.Error("Failed to create client")
		return false, err
	}
	...
	if err != nil {
		acelog.Error("Failed to get Node ID")
		return false, err
	}

	if len(ID) == 0 {
		acelog.Debug("Not a part of swarm, so Node ID is empty")
		return false, nil
	}

	node, _, err := cli.NodeInspectWithRaw(context.Background(), ID)
	if err != nil {
		acelog.Error("NodeInspect with Raw:", err)
		return false, nil
	}
...
func DemoteNode(ID string) error {
	...
	if err != nil {
		acelog.Error("Failed to update the node")
		return err
...
func RemoveNode(ID string, force bool) error {
	...
	if err != nil {
		acelog.Error("Failed to remove the node")
		return err
...
func GetToken(input string) (string, error) {
	...
	acelog.Debug("Pass the right input")
...
func GetSystemDockerNode(name string) (string, error) {
	...
	if err != nil {
		acelog.Error("failed to get client")
		return "", err
	}
	...
	if err != nil {
		acelog.Error("failed to get client list")
		return "", err
	}
	...
	acelog.Debug("There is no container name ", name)
	return "", nil
...
func RestartSystemDockerContainer(name string) error {
	...
	cli, err := client.NewClientWithOpts(client.FromEnv, client.WithHost(sock))
	if err != nil {
		acelog.Error("failed to get client", err.Error())
		return err
	}
	...
	containers, err := cli.ContainerList(context.Background(), types.ContainerListOptions{})
	if err != nil {
		acelog.Error("failed to get client list", err.Error())
		return err
	}
...
func RemoveSystemDockerNode(ID string) error {
	...
	if err != nil {
		acelog.Error("Failed to create new client")
		return err
	}
	...
	for _, image := range images {
		...
		for i := 0; i < len(tags); i++ {
			if strings.Contains(tags[i], "dind") {
				...
				}, &container.HostConfig{Privileged: true}, nil, "ace_arbiter")
				if err != nil {
					acelog.Error(err)
					return err
				}
...
func ExecuteProcessInNode(ID string, cmd []string) (string, error) {
	...
	if err != nil {
		acelog.Error("Failed to create the client")
		return "", err
	}
	defer cli.Close()
	...
	if err != nil {
		acelog.Error("Failed to Exec create", err)
		return "", err
	}
	...
	if err != nil {
		acelog.Error("Failed to Exec Attach")
		return "", err
	}
...
func WaitForDocker() bool {
	...
	for {
		if retry < 200 {
			...
			if err != nil {
				acelog.Error("Error while connecting to Docker  ", err.Error())
				retry++
...
func RestartGlusterContainer() error {
	acelog.Debug("Restart the gluster container")
	err := RestartSystemDockerContainer("gluster-server")
	if err != nil {
		acelog.Error("Error while restarting gluster container ID")
	}
	return nil
}
...
func RemoveDockerNodes() error {
	...
	if err != nil {
		acelog.Debug("Error while retrieving the docker nodes which are down ", err.Error())
		return err
	}

	for _, element := range node {
		if len(element) > 0 {
			acelog.Debug("Removing member from docker swarm cluster " + element)

			acelog.Debug("DemoteNode " + element)
			err = DemoteNode(element)
			if err != nil {
				acelog.Debug("Error in demoting a docker member. ", err.Error())
			...
			acelog.Debug("RemoveNode " + element)
			err := RemoveNode(element, true)
			if err != nil {
				acelog.Debug("Error while removing docker node ", err.Error())
				return err
			}
		}
	}
