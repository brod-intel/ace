...
// SetWaitingForWorkerTag set waitingfoacerker Tag for the serf member.
func SetWaitingForWorkerTag(value string) error {

	tag := make(map[string]string)
	tag["waitingfoacerker"] = value
	return setSerfTag(tag)
}
...
func GetNodeIDForWorkerWithMinTagValue() (string, error) {
	...
	reachableNodeIDs, err := SerfQuery("godocker", "reachableNodeIDs")
	if err != nil {
		acelog.Error("Node Failed to check the ReachableNodeCount ", err)
		return "", err
	}

	acelog.Error("reachablenodeinfo", reachableNodeIDs) //Debug Info

	for i := 0; i < len(members); i++ {

		//Avoid the value for leader node
		if members[i].Tags["role"] == "leader" {
			acelog.Debug("Leader return\n")
			continue
		}

		//If reachable node avoid the init value for it.
		if strings.Contains(reachableNodeIDs, members[i].Tags["swarm"]) {
			acelog.Debug("Avoiding init for reachable node")
			continue
		}
		...
		acelog.Debug("init array :", initArray[initCount])
		initCount++
	...
	acelog.Debug("sorted init:", initArray) //Debug Info

	//Return the swarmid to be promoted
	for i := 0; i < len(members); i++ {

		if len(initArray) == 0 {
			acelog.Debug("No Value in init, returning back")
			continue
		}

		tmp, _ := strconv.Atoi(members[i].Tags["init"])
		acelog.Debug("Member init  :", tmp)
		acelog.Debug("Member swarm id:", members[i].Tags["swarm"])
		...
		if tmp == initArray[0] {
			...
			acelog.Debug("SwarmNodeID from docker API :", swarmID)

			if swarmID == members[i].Tags["swarm"] {
				acelog.Debug("Matched") //Debug
...
func MemberForceLeave(MemberName string) error {
	...
	if err != nil {
		acelog.Error("Error while removing failed node", err.Error())
		return err
	}
...
func setSerfTag(tag map[string]string) error {
	...
	if err != nil {
		acelog.Error("Error while setting up serf tags", err.Error())
		return err
	}
	return nil
}
...
func query(name string, payload string, timeout int) (string, error) {
	...
	case r := <-respCh:
		if r.From == "" {
			acelog.Debug("Serf query response from leader is empty. This may signify that leader might be busy executing member-failed. Retrying ...... ")
			return "", nil
		}
		payload := r.Payload
