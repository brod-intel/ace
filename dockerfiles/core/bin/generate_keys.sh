...
printBanner $msg
logMsg $msg
mkdir -p /opt/ace/tmp/node_ca_key
cd /opt/ace/tmp/node_ca_key
/opt/ace/bin/credtool_create_ca.sh ace
/opt/ace/bin/credtool_create_keys.sh ace.key ace.crt

msg="Generating Cluster Key..."
printBanner $msg
logMsg $msg
mkdir -p /opt/ace/node_keys
param_acekey="$(tr </dev/urandom -dc +a-fA-F0-9 | head -c43)="
echo '[ \"${param_acekey}\" ]' > /opt/ace/node_keys/keyring.json
